...
const checked = computed({
  get: () => isGroup.value ? group.modelValue.value.includes(props.label) : props.modelValue,
  set: (v: boolean) => {
...
<template>
  <label :class="[bem.b(), bem.is('disabled', disabled || group?.disabled?.value), bem.is('indeterminate', indeterminate)]">
    <input ref="inputRef" type="checkbox" :class="bem.e('input')" v-model="checked" :disabled="disabled || group?.disabled?.value" :aria-checked="ariaChecked" />
